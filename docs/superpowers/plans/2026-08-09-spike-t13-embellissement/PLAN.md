...
async function findInboxImage(stem: string): Promise<string> {
  ...
  throw new Error(
    `Image ${stem} introuvable dans ${INBOX_DIRECTORY} (JPEG attendu).`,
  )
}
...
describe('LADDER', () => {
  ...
  it('excludes the -preview duplicates and the auto routers', () => {
    ...
    expect(models.some((model) => model.startsWith('openrouter/auto'))).toBe(
      false,
    )
  })
  ...
  it('caps every call above its catalogue rate, so the budget guard is never optimistic', () => {
    for (const rung of LADDER) {
      expect(rung.maxCostUsdPerCall).toBeGreaterThan(
        rung.imageOutputUsdPerToken * 1000,
      )
    }
  })

  it('keeps the whole grid affordable: 4 models x 4 dishes x 2 passes under the 10 USD cap', () => {
    const worstCase = LADDER.reduce(
      (total, rung) => total + rung.maxCostUsdPerCall * 8,
      0,
    )
...
describe('modelSlug', () => {
  it('turns a model id into a single path segment', () => {
    expect(modelSlug('google/gemini-2.5-flash-image')).toBe(
      'google__gemini-2.5-flash-image',
    )
  })
})
  ...
  it('reports a text-only answer and keeps the text as the detail', () => {
    const decoded = decodeImageResponse({
      choices: [
        {
          finish_reason: 'stop',
          message: { content: 'Voici une description du plat.' },
        },
      ],
    })
    ...
    expect(decoded).toHaveProperty(
      'detail',
      expect.stringContaining('description du plat'),
    )
  })
      ...
      choices: [
        {
          finish_reason: 'stop',
          message: {
            images: [{ image_url: { url: 'https://example.test/x.png' } }],
          },
        },
      ],
    })
...
describe('renderImage', () => {
  it('records the real cost reported by the API, not the estimate', async () => {
    ...
    const result = await renderImage({
      ...
      maxCostUsd: 0.5,
      fetchImpl: async () =>
        new Response(JSON.stringify(responseWithImage()), { status: 200 }),
    })
...
export type DecodeFailure = {
  status: 'failure'
  reason: 'refusal' | 'truncation' | 'no_image'
  detail: string
}
...
export function decodeImageResponse(
  raw: OpenRouterImageResponse,
): DecodedImage | DecodeFailure {
  ...
  if (typeof url === 'string') {
    ...
    return {
      status: 'failure',
      reason: 'no_image',
      detail: `URL d'image non décodable : ${url.slice(0, 120)}`,
    }
  }
  ...
  if (choice?.finish_reason === 'length') {
    return {
      status: 'failure',
      reason: 'truncation',
      detail: 'finish_reason=length sans image.',
    }
  }
...
export type RenderSuccess = DecodedImage & {
  latencyMs: number
  actualCostUsd: number
  raw: OpenRouterImageResponse
}
export type RenderFailure = DecodeFailure & {
  latencyMs: number
  actualCostUsd: number
  raw: OpenRouterImageResponse
}
...
export function requireOpenRouterApiKey(
  ...
  if (!key) {
    throw new Error(
      'OPENROUTER_API_KEY manque. Renseigne .env avec une clé OpenRouter avant tout appel payant.',
    )
  }
  return key
}
...
function isTransient({
  ...
  return (
    status === 429 ||
    status >= 500 ||
    /rate.?limit|temporar|timeout|timed out|overloaded/i.test(message)
  )
}
...
export async function renderImage({
  ...
  try {
    ...
    const cost =
      typeof raw.usage?.cost === 'number' && Number.isFinite(raw.usage.cost)
        ? raw.usage.cost
        : 0
...
describe('buildGrid', () => {
  it('pairs every model with every dish, twice', () => {
    expect(buildGrid(['recadre1', 'recadre2', 'brut1', 'brut2'])).toHaveLength(
      32,
    )
  })
...
describe('renderPath', () => {
  ...
  it('follows the media type the model actually returned', () => {
    ...
    ).toBe('spike13/fixtures/renders/openai__gpt-5.4-image-2/brut1-1.jpg')
  })
})
...
describe('sidecarPath', () => {
  it('sits next to the render and carries the outcome even when no image came back', () => {
    ...
    ).toBe('spike13/fixtures/renders/openai__gpt-5-image-mini/recadre1-1.json')
  })
})
```
...
async function main(): Promise<void> {
  ...
  const costByModel = new Map(
    LADDER.map((rung) => [rung.model, rung.maxCostUsdPerCall]),
  )

  console.log(
    `Grille : ${grid.length} cellules · déjà dépensé ${budget.spent.toFixed(4)} USD / ${budget.cap} USD.`,
  )

  for (const cell of grid) {
    if (await alreadyDone(cell)) {
      console.log(
        `− ${cell.model} ${cell.dish} passe ${cell.pass} : déjà fait, non rappelé.`,
      )
      continue
    }
    ...
    if (maxCostUsd === undefined)
      throw new Error(`Modèle hors échelle : ${cell.model}.`)
