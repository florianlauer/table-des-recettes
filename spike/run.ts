...
export function endpointFromLadder({
  ...
  const endpoint = ladder.ladder.find(
    (entry) => entry.model === model && entry.providerSlug === providerSlug,
  )
  if (!endpoint) {
    throw new Error(
      `Couple ${model} / ${providerSlug} absent de l'échelle figée.`,
    )
  }
  return endpoint
}
...
export async function runEscalationEndpoint({
  ...
  for (const page of pages) {
    for (const pass of [1, 2]) {
      ...
      console.log(
        `${page.toUpperCase()} passe ${pass} : ${result.status} (${result.attempts} tentative(s)).`,
      )
    }
  }
}
...
async function main(): Promise<void> {
  ...
  const pages = requestedPages
    ?.split(',')
    .map((page) => safePathPart(page.trim()))
    .filter(Boolean)
