...
export function decideRung({
  ...
  if (result.status === "failure") {
    return { verdict: "echec_modele", detail: `${result.reason} : ${result.detail}` };
  }
...
export async function loadCondemnedModels(path: string): Promise<Map<string, string>> {
  try {
    const entries = JSON.parse(await readFile(path, "utf8")) as Array<{ model: string; reason: string }>;
    return new Map(entries.map(({ model, reason }) => [model, reason]));
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "ENOENT") return new Map();
    throw new Error(`Liste des modèles condamnés illisible (${path}) : ${String(error)}`);
  }
}

async function alreadyProbed({ model, providerSlug }: { model: string; providerSlug: string }): Promise<boolean> {
  try {
    const files = await readdir(resolve("spike/fixtures/runs", model, providerSlug));
...
async function latestLadder(): Promise<LadderFile> {
  const latest = (await readdir("spike"))
    ...
    .sort()
    .at(-1);
  if (!latest) throw new Error("Aucune échelle figée. Exécutez d'abord npm run rank.");
  return JSON.parse(await readFile(resolve("spike", latest), "utf8")) as LadderFile;
}

async function writeHarnessFailure({ endpoint, message }: { endpoint: LadderEntry; message: string }): Promise<void> {
  const directory = resolve("spike/fixtures/runs", endpoint.model, endpoint.providerSlug);
...
async function main(): Promise<void> {
  ...
  if (!Number.isInteger(startRank) || startRank < 1) throw new Error("--from attend un rang entier ≥ 1.");
  if (!(maximumLatencySeconds > 0)) throw new Error("--max-latency attend un nombre de secondes > 0.");
  if (!Number.isInteger(maximumRungs) || maximumRungs < 1) throw new Error("--limit attend un entier ≥ 1.");
  ...
  let probed = 0;
  for (let index = startRank - 1; index < ladder.ladder.length && probed < maximumRungs; index += 1) {
    ...
    if (decision.verdict === "retenu") {
      console.log(`${label} — RETENU en ${decision.latencySeconds.toFixed(1)} s (${result?.actualCostUsd ?? 0} USD).`);
      console.log(
    ...
    if (decision.verdict === "trop_lent") {
      console.log(`${label} — trop lent : ${decision.latencySeconds.toFixed(1)} s > ${maximumLatencySeconds} s.`);
    } else {
      console.log(`${label} — ${decision.verdict} : ${decision.detail.slice(0, 160)}`);
      if (result && condemnsWholeModel(result)) {
        condemned.set(endpoint.model, decision.detail.slice(0, 160));
        console.log(`  → les autres endpoints de ${endpoint.model} sont écartés de cette marche.`);
      }
    }
  }
