...
describe("ingestion", () => {
  ...
  it("applique l'orientation EXIF puis retire toutes les métadonnées", async () => {
    const inputPath = resolve(directory, "oriented.jpg");
    const outputPath = resolve(directory, "normalized.jpg");
    await sharp({ create: { width: 10, height: 20, channels: 3, background: "red" } })
  ...
  it("n'agrandit pas une petite image", async () => {
    ...
    await sharp({ create: { width: 120, height: 80, channels: 3, background: "white" } }).jpeg().toFile(inputPath);
    const outputMetadata = await normalizeImage({ inputPath, outputPath });
