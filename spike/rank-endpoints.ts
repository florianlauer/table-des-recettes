...
function cost({
  ...
  const imageCost =
    pricing.imagePerImage ?? pricing.prompt * IMAGE_TOKENS_ESTIMATE
  return (
...
async function getJson<T>({
  ...
  if (!response.ok) {
    throw new Error(
      `OpenRouter ${response.status} sur ${url} : ${await response.text()}`,
    )
  }
...
export async function discoverEndpoints({
  ...
  known: LadderEntry[]
  uncertain: UncertainEndpoint[]
  excluded: LadderFile['excluded']
}> {
  ...
  const models = (modelsResponse.data ?? []).filter(
    (model): model is ApiModel & { id: string } =>
      typeof model.id === 'string' && modalities(model).includes('image'),
  )
  ...
  for (const model of models) {
    ...
    const endpointsResponse = await getJson<{
      data?: { endpoints?: ApiEndpoint[] } | ApiEndpoint[]
    }>({
    ...
    for (const endpoint of endpoints) {
      ...
      if (!slug || !displayName) {
        excluded.push({
          model: model.id,
          providerSlug: 'inconnu',
          reason: 'identifiant de provider absent',
        })
        continue
      }
      const supportsImage =
        modalities(endpoint).includes('image') ||
        modalities(model).includes('image')
      const parameters = new Set(endpoint.supported_parameters ?? [])
      const supportsRequest = REQUIRED_PARAMETERS.every((parameter) =>
      ...
      if (!supportsImage || !supportsRequest) {
        excluded.push({
          model: model.id,
          providerSlug: slug,
          reason: 'modalité image ou paramètres stricts absents',
        })
        continue
      }
      ...
      known.push({
        ...
        rankingCostUsd: cost({
          pricing,
          completionTokens: REPRESENTATIVE_COMPLETION_TOKENS,
        }),
        maximumCallCostUsd: cost({
          pricing,
...
export async function buildLadder({
  ...
  const { known, uncertain, excluded } = await discoverEndpoints({
    apiKey,
    fetchImpl,
  })
  const notEvaluated: LadderFile['notEvaluated'] = []
  const queue = [...uncertain].sort(
  ...
  console.log(
    `Budget pire cas avant le premier appel payant : ${worstCaseBeforePaidCalls.toFixed(6)} USD.`,
  )
  ...
  for (const [index, endpoint] of queue.entries()) {
    if (index >= MAX_UNCERTAIN_PROBES) {
      notEvaluated.push({
        ...endpoint,
        reason: 'file de trois sondes déjà pleine',
      })
      continue
    }
    try {
    ...
    if (result.status !== 'success') {
      notEvaluated.push({
        ...endpoint,
        reason: `sonde impossible : ${result.detail}`,
      })
      continue
    }
    if (result.actualCostUsd > PROBE_RESERVE_USD) {
      excluded.push({
        ...endpoint,
        reason: `sonde ${result.actualCostUsd} USD > réserve ${PROBE_RESERVE_USD} USD`,
      })
      continue
    }
...
async function main(): Promise<void> {
  ...
  const worstCase =
    result.ladder.reduce(
      (sum, endpoint) => sum + endpoint.maximumCallCostUsd * 24,
      0,
    ) +
    Math.min(MAX_UNCERTAIN_PROBES, result.notEvaluated.length) *
      PROBE_RESERVE_USD
  console.log(
    `Budget pire cas avant escalade : ${worstCase.toFixed(6)} USD ; dépensé : ${budget.spent.toFixed(6)} USD.`,
  )
