...
function failure(reason: "invalid_schema" | "unsupported_request"): PassResult {
  return { status: "failure", reason, detail: "detail", attempts: 1, latencyMs: 900, actualCostUsd: 0.0005 };
}
...
describe("ladder walk", () => {
  ...
  it("keeps a rung whose probe lands under the latency floor", () => {
    expect(decideRung({ result: success(2_100), maximumLatencySeconds: 10 })).toEqual({
      verdict: "retenu",
      latencySeconds: 2.1,
    });
  });
  ...
  it("drops a rung that is too slow even when the extraction succeeds", () => {
    expect(decideRung({ result: success(84_000), maximumLatencySeconds: 10 })).toMatchObject({ verdict: "trop_lent" });
  });

  it("separates a model failure from a transient one", () => {
    expect(decideRung({ result: failure("invalid_schema"), maximumLatencySeconds: 10 })).toMatchObject({
      verdict: "echec_modele",
    });
    expect(
      decideRung({
        result: { status: "inconclusive", reason: "transient_error", detail: "AbortError", attempts: 3, latencyMs: 361_000, actualCostUsd: 0 },
  ...
  it("treats a rung answering exactly at the floor as kept", () => {
    expect(decideRung({ result: success(10_000), maximumLatencySeconds: 10 })).toMatchObject({ verdict: "retenu" });
  });
