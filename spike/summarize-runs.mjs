...
const shape = (extraction) =>
  extraction.recipes
    .map((r) => `${r.ingredients.length}i/${r.steps.length}e`)
    .join(' ')
...
for (const [model, provider, note] of MODELS) {
  ...
  for (const page of PAGES) {
    ...
    if (one.status !== 'success') {
      failures += 1
      rows.push(
        `  ${page.toUpperCase()}  ÉCHEC ${one.reason ?? ''} ${(one.detail ?? '').slice(0, 70)}`,
      )
      continue
    }
    ...
    const same =
      two?.status === 'success' &&
      JSON.stringify(one.parsed) === JSON.stringify(two.parsed)
    if (!same) unstable += 1
    rows.push(
