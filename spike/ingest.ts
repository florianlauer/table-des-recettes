...
async function main(): Promise<void> {
  ...
  if (!page || !/^[a-z][a-z0-9-]*$/i.test(page)) {
    throw new Error(
      "Usage : npm run ingest -- <role> [source] (ex. « B duo1 », ou « A » si le fichier s'appelle déjà A).",
    )
  }
  const inputPath = await findInboxImage(process.argv[3] ?? page)
  const outputPath = resolve(
    'spike/fixtures/pages',
    `${page.toLowerCase()}.jpg`,
  )
  ...
  console.log(
    'Reporte la correspondance rôle/source et le nombre réel de recettes dans spike/fixtures/pages/README.md.',
  )
}
