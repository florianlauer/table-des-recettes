...
import { renderImage, requireOpenRouterApiKey } from './openrouter.js'
import type { RenderResult } from './openrouter.js'
...
export function assertKnownFilters({
  ...
  for (const dish of dishes) {
    if (!(DISHES as readonly string[]).includes(dish)) {
      throw new Error(
        `Plat inconnu : ${dish}. Attendus : ${DISHES.join(', ')}.`,
      )
    }
  }
  ...
  for (const model of models) {
    if (!LADDER.some((rung) => rung.model === model)) {
      throw new Error(
        `Modèle hors échelle : ${model}. Attendus : ${LADDER.map((rung) => rung.model).join(', ')}.`,
      )
    }
  }
}
...
async function alreadyDone(cell: Cell): Promise<boolean> {
  try {
    const sidecar = JSON.parse(await readFile(sidecarPath(cell), 'utf8')) as {
      status?: unknown
    }
