...
describe('parseCellFilters', () => {
  ...
  it('reads --pass as a number so it can match a cell', () => {
    expect(parseCellFilters(['--pass', '1'])).toEqual({
      dishes: [],
      passes: [1],
      models: [],
    })
...
describe('assertKnownFilters', () => {
  it('accepts a dish that exists', () => {
    expect(() =>
      assertKnownFilters({ dishes: ['recadre1'], passes: [1], models: [] }),
    ).not.toThrow()
  })

  it('rejects a misspelled dish instead of silently selecting nothing', () => {
    expect(() =>
      assertKnownFilters({ dishes: ['recadre3'], passes: [], models: [] }),
    ).toThrow(/Plat inconnu/)
  })

  it('rejects a pass that is neither 1 nor 2', () => {
    expect(() =>
      assertKnownFilters({ dishes: [], passes: [3], models: [] }),
    ).toThrow(/Passe inconnue/)
  })
...
describe('selectCells', () => {
  it('keeps every cell when no axis is filtered', () => {
    ...
    expect(
      selectCells({ cells, dishes: [], passes: [], models: [] }),
  ...
  it('narrows to the named models, so an eliminated model is not paid for again', () => {
    ...
    expect(selected.every((cell) => cell.model.startsWith('google/'))).toBe(
      true,
    )
  })
  ...
  it('screens the whole ladder on one dish and one pass — four cells, one per model', () => {
    ...
    expect(
      selected.every((cell) => cell.dish === 'recadre1' && cell.pass === 1),
    ).toBe(true)
  })
})
...
describe('renderPath', () => {
  it('gives every render its own path, one directory per model', () => {
    expect(
      ...
      'spike13/fixtures/renders/google__gemini-2.5-flash-image/recadre1-2-v2.png',
    )
  })
  ...
  it('keeps a v2 render apart from the v1 render of the same cell', () => {
    ...
    expect(renderPath({ ...cell, promptVersion: 'v1' })).not.toBe(
      renderPath({ ...cell, promptVersion: 'v2' }),
    )
  })
